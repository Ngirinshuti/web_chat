...
if (isset($_POST['getMessageStory']) && isset($_POST['storyId'])) {
    exit(json_encode($msg_obj->getStory($_POST['storyId'])));
}
...
<head>
    ...
    <style>
        ...
        .user-messages-wrapper .replyWrapper {
            left: 75px;
            bottom: 0;
        }
...
<script>
    ...
    async function getUserStoryElement(story_id) {
        const storyElement = document.createElement('div');
        storyElement.classList.add("replyWrapper")

        const body = new FormData();
        body.append('getMessageStory', 'true')
        body.append('storyId', story_id)
        ...
        storyElement.innerHTML = `
            <a href='../stories/?story=${story.id}' class="replyLink">
            ${story.image ? `<img src='../stories/story_uploads/${story.image}' class='replyImg' />` : '' }\
            ${story.description ? `<p class='replyDesc'>${sliceLong(story.description)}</p>` : '' }
            </a>
        `
        ...
        for (var i in messages) {
            ...
            if (msg.body.trim() !== '') {
                ...
                if (!msg.story_id) {
                    ...
                    msgContainer.appendChild(senderImage);
                    msgContainer.classList.add("has_story");
                    (!bl) ? msgContainer.appendChild(msgBody): null;
                    addDate = true;
                }
                if (msg.story_id) {
                    msgContainer.appendChild(await getUserStoryElement(msg.story_id))
                }
            }
